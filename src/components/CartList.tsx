...
export const CartList: React.FC<CartListProps> = ({ onClose }) => {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'ussd' | 'mcx_express' | 'bank_transfer' | 'cash' | 'reference'>('mcx_express');
  const [mcxPhoneNumber, setMcxPhoneNumber] = useState('');
  ...
  const updateQuantity = async (id: string, newQty: number) => {
    ...
    try {
      ...
      handleFirestoreError(error, OperationType.UPDATE, `cart/${id}`);
    }
  };
  ...
  const removeItem = async (id: string) => {
    try {
      ...
      handleFirestoreError(error, OperationType.DELETE, `cart/${id}`);
    }
  };
    ...
    try {
      ...
      for (const farmerId of farmers) {
        ...
        const orderData = {
          ...
          paymentMethod,
          paymentStatus: paymentMethod === 'mcx_express' ? 'initiated' : 'pending',
        ...
        try {
          ...
          orderIdCreated = docRef.id;
        } catch (err) {
          handleFirestoreError(err, OperationType.CREATE, 'orders');
        }

        // If MCX Express, call the backend API
        if (paymentMethod === 'mcx_express' && orderIdCreated) {
          try {
            const response = await fetch('/api/payments/mcx-express', {
              method: 'POST',
            ...
            console.log('MCX Express response:', data);
          } catch (err) {
            console.error('Failed to initiate MCX Express payment:', err);
          }
        }
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {[
                        { id: 'mcx_express', title: 'Multicaixa Express', icon: Smartphone, color: 'blue', desc: 'Pagamento rápido via telemóvel' },
                        { id: 'ussd', title: 'Unitel Money', icon: Smartphone, color: 'orange', desc: 'Pagamento via USSD' },
                    ...
                    {paymentMethod === 'mcx_express' && (
                      <motion.div 
                        ...
                        className="mt-4 p-6 bg-blue-50 rounded-2xl border-2 border-blue-100 space-y-4"
                      >
                        <div className="flex items-center gap-2 text-blue-700 mb-2">
                          <Info className="w-4 h-4" />
                          <span className="text-xs font-black uppercase tracking-widest">Número Multicaixa Express</span>
                        </div>
                        <div className="space-y-2">
                          <input 
                            type="tel"
                            value={mcxPhoneNumber}
                            onChange={(e) => setMcxPhoneNumber(e.target.value)}
                            placeholder="9XXXXXXXX"
                            className="w-full px-4 py-4 bg-white border-2 border-blue-100 rounded-2xl focus:border-blue-600 outline-none transition-all font-black text-lg tracking-widest"
                          />
