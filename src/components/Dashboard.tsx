...
import { collection, query, where, onSnapshot, addDoc, serverTimestamp, updateDoc, doc, deleteDoc, orderBy, getDocs, increment, getDoc } from 'firebase/firestore';
...
export const Dashboard: React.FC = () => {
  ...
  const handleAddProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'products'), {
        ...
        farmerVerificationStatus: profile?.verificationStatus || 'unverified',
        farmerRating: profile?.reputation?.rating || 0,
        farmerReviewCount: profile?.reputation?.reviewCount || 0,
        status: 'available',
  ...
  const submitReview = async () => {
    ...
    try {
      const targetId = reviewingOrder.farmerId; // For now, review the farmer
      
      // 1. Create the review document
      await addDoc(collection(db, 'reviews'), {
        orderId: reviewingOrder.id,
        reviewerId: user.uid,
        reviewerName: profile?.name || user.displayName,
        targetId: targetId,
        rating: reviewRating,
      ...
      // 2. Update target user's reputation
      const targetRef = doc(db, 'users', targetId);
      const targetSnap = await getDoc(targetRef);
      if (targetSnap.exists()) {
        const targetData = targetSnap.data();
        const currentSum = targetData.reputation?.totalRatingSum || 0;
        const currentCount = targetData.reputation?.reviewCount || 0;
        ...
        await updateDoc(targetRef, {
          'reputation.totalRatingSum': increment(reviewRating),
          'reputation.reviewCount': increment(1),
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      ...
      <aside className="w-full md:w-72 bg-white border-r border-gray-100 p-8 flex flex-col gap-8 sticky top-20 h-[calc(100vh-80px)] overflow-y-auto">
        <div className="space-y-2">
          <div className="px-4 py-6 flex items-center gap-3 border-b border-gray-50 mb-4">
            <div className="w-12 h-12 bg-green-100 rounded-2xl flex items-center justify-center text-green-600 font-black text-xl shadow-sm">
              {profile?.name?.charAt(0) || user?.email?.charAt(0).toUpperCase()}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                <p className="font-black text-gray-900 text-sm truncate">{profile?.name || 'Utilizador'}</p>
              ...
              <div className="flex items-center gap-1 mt-0.5">
                <div className="flex text-yellow-400">
                  {[...Array(5)].map((_, i) => (
                    <Star 
                      key={i} 
                      className={`w-2.5 h-2.5 ${i < Math.floor(profile?.reputation?.rating || 0) ? 'fill-current' : 'text-gray-300'}`} 
                    />
                  ))}
                </div>
                <span className="text-[9px] font-bold text-gray-400">({profile?.reputation?.reviewCount || 0})</span>
              </div>
            </div>
          </div>
          ...
          {[
            ...
            { id: 'verification', label: 'Verificação', icon: ShieldCheck, badge: profile?.verificationStatus === 'unverified' ? '!' : undefined },
            ...
            >
              ...
              {item.badge && (
                <span className="bg-orange-500 text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center animate-pulse">
                  {item.badge}
                </span>
              )}
            </button>
                      ...
                      <div key={o.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100">
                        ...
                        <div className="text-right">
                          ...
                          <div className="flex flex-col items-end gap-1">
                            ...
                            {o.status === 'delivered' && !o.reviewed && profile?.role === 'buyer' && (
                              <button 
              ...
              <div className="space-y-8">
                ...
                <div className="pt-12 border-t border-gray-100">
                  <h3 className="text-xl font-black text-gray-900 mb-8 flex items-center gap-2">
                    <Shield className="w-6 h-6 text-green-600" /> Segurança e Confiança
                  </h3>
                  ...
                  <div className="space-y-6">
                    <div className="flex items-center justify-between p-6 bg-gray-50 rounded-3xl border border-gray-100">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h4 className="font-black text-gray-900 text-sm">Autenticação de Dois Factores (2FA)</h4>
                          <p className="text-xs text-gray-500 font-medium">Adicione uma camada extra de segurança à sua conta.</p>
                        </div>
                      </div>
                      <button className="w-12 h-6 bg-gray-200 rounded-full relative transition-colors hover:bg-green-200">
                        <div className="absolute left-1 top-1 w-4 h-4 bg-white rounded-full shadow-sm"></div>
                      </button>
                    </div>

                    {(profile?.role === 'farmer' || profile?.role === 'transporter') && (
                      <div className="flex items-center justify-between p-6 bg-gray-50 rounded-3xl border border-gray-100">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h4 className="font-black text-gray-900 text-sm">Dados Financeiros</h4>
                            <p className="text-xs text-gray-500 font-medium">IBAN: {profile?.verificationData?.iban || 'Não configurado'}</p>
                          </div>
                        </div>
                        <button 
                    ...
                    <div className="flex items-center justify-between p-6 bg-gray-50 rounded-3xl border border-gray-100">
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                          profile?.verificationStatus === 'verified' ? 'bg-green-100 text-green-600' : 'bg-orange-100 text-orange-600'
                        }`}>
                          <ShieldCheck className="w-6 h-6" />
                        </div>
                        <div>
                          <h4 className="font-black text-gray-900 text-sm">Estado de Verificação</h4>
                          <p className="text-xs text-gray-500 font-medium uppercase tracking-widest font-black">
                            {profile?.verificationStatus === 'verified' ? 'Verificado' : 'Pendente / Não Verificado'}
                          </p>
                        </div>
                      </div>
                      <button 
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-black text-gray-900 tracking-tight">Avaliar Produtor</h3>
                <button onClick={() => setReviewingOrder(null)} className="p-2 hover:bg-gray-100 rounded-full transition-colors">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">O seu comentário (opcional)</label>
                  <textarea
                    value={reviewComment}
                    onChange={(e) => setReviewComment(e.target.value)}
                    className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-green-500 outline-none resize-none h-32"
